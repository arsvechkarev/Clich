package com.arsvechkarev.core.recyler.listadapter;
...
/**
 * Helper for computing the difference between two lists via {@link DiffUtil} on a
 * background thread.
 * <p>
 * It can be connected to a {@link RecyclerView.Adapter RecyclerView.Adapter}, and will
 * signal the adapter of changes between sumbitted lists.
 * <p>
 * For simplicity, the {@link BaseAdapter} wrapper class can often be used instead of the
 * AsyncListDiffer directly. This AsyncListDiffer can be used for complex cases, where
 * overriding an adapter base class to support asynchronous List diffing isn't
 * convenient.
 * <p>
 * The AsyncListDiffer can consume the values from a LiveData of <code>List</code> and
 * present the data simply for an adapter. It computes differences in list contents via
 * {@link DiffUtil} on a background thread as new <code>List</code>s are received.
 * <p>
 * Use {@link #getCurrentList()} to access the current List, and present its data objects.
 * Diff results will be dispatched to the ListUpdateCallback immediately before the
 * current list is updated. If you're dispatching list updates directly to an Adapter,
 * this means the Adapter can safely access list items and total size via {@link
 * #getCurrentList()}.
 * <p>
...
public class AsyncListDiffer<T> {
  ...
  public interface ListListener<T> {
     ...
     */
    void onCurrentListChanged(@NonNull List<T> previousList,
  ...
  /**
   * Convenience for {@code AsyncListDiffer(new AdapterListUpdateCallback(adapter), new
   * AsyncDifferConfig.Builder().setDiffCallback(diffCallback).build());}
   *
   * @param adapter Adapter to dispatch position updates to.
   * @param diffCallback ItemCallback that compares items to dispatch appropriate
   * animations when
   ...
   * @param config Config to define background work Executor, and DiffUtil.ItemCallback
   * for computing List diffs.
   * @see DiffUtil.DiffResult#dispatchUpdatesTo(RecyclerView.Adapter)
   */
   ...
   * <p>
   * Collections.emptyList when mList is null, wrapped by Collections.unmodifiableList
   * otherwise
   */
  @NonNull
   ...
   * <p>
   * If a <code>null</code> List, or no List has been submitted, an empty list will be
   * returned.
   * <p>
  ...
  /**
   * Changes list without animation or calculating updates
   *
   * @param newList New list to be changed to
   */
  public void changeListQuietly(List<T> newList) {
    mList = newList;
    mReadOnlyList = Collections.unmodifiableList(newList);
  }

  /**
   * Pass a new List to the AdapterHelper. Adapter updates will be computed on a
   * background thread.
   * <p>
   * If a List is already present, a diff will be computed asynchronously on a background
   * thread. When the diff is computed, it will be applied (dispatched to the {@link
   * ListUpdateCallback}), and the new List will be swapped in.
   *
